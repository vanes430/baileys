...
export const makeMessagesRecvSocket = (config) => {
    const { logger, retryRequestDelayMs, maxMsgRetryCount, getMessage, shouldIgnoreJid, enableAutoSessionRecreation, rejectCalls } = config;
    ...
    const handleCall = async (node) => {
        ...
        if (status === 'offer') {
            ...
            if (rejectCalls) {
                logger.info({ callId: call.id, from: call.from }, 'rejecting incoming call automatically');
