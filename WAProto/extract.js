 ...
 * Modified for Baileys ESM integration
 */
...
async function fetchText(url) {
  const response = await fetch(url, { headers: ua.headers });
  if (!response.ok) {
...
async function findAppModules() {
  ...
  const qrData = await fetchText(bootstrapQRURL);
  ...
  for (const mod of modules) {
    ...
    const stringifyMessageSpecMember = (
      ...
      if (info.type === '__oneof__') {
        ...
        // Convert 'required' flag to 'optional' since we are using proto3
        info.flags = info.flags.map((f) => f === 'required' ? 'optional' : f);
